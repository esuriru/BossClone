...
/*!
\brief
operator* overload for matrix-glm::vec3 multiplication

\param rhs
	glm::vec3 to multiply with
\return 
	Resulting vector
*/
/******************************************************************************/
glm::vec3 Mtx44::operator*(const glm::vec3& rhs) const {
	float b[4];
	for(int i = 0; i < 4; i++)
		b[i] = a[0 * 4 + i] * rhs.x + a[1 * 4 + i] * rhs.y + a[2 * 4 + i] * rhs.z + a[3 * 4 + i] * 0;
	glm::vec3 ret(b[0], b[1], b[2]);
	return ret;
}

void Mtx44::SetToEulerAngles(const glm::vec3& rhs)
{
	if (rhs.LengthSquared() < Math::EPSILON * Math::EPSILON) return;
	auto normalizedRotation = rhs.Normalized();
	SetToRotation(rhs.Length(), normalizedRotation.x, normalizedRotation.y, normalizedRotation.z);
}
...
void Mtx44::SetToLookAt(double eyeX, double eyeY, double eyeZ,
	...
	glm::vec3 f((float)(centerX - eyeX), (float)(centerY - eyeY), (float)(centerZ - eyeZ));
	f.Normalize();
	glm::vec3 up((float)upX, (float)upY, (float)upZ);
	up.Normalize();
	glm::vec3 s = f.Cross(up);
	glm::vec3 u = s.Cross(f);
